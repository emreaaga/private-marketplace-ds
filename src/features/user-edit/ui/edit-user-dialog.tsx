...
import { COUNTRY_META, CountryCode } from "@/entities/geography";
import { getUserDetailApi, type UserDetail } from "@/entities/user";
import { usersKeys } from "@/entities/user/queries/users.keys";
...
export function EditUserDialog({ open, userId, onOpenChange, onSubmitAction }: Props) {
  ...
  const { data, isLoading, isError } = useQuery<UserDetail>({
    ...
    enabled,
    queryFn: ({ signal }) => getUserDetailApi(userId as number, signal),
    staleTime: 30_000,
  });
